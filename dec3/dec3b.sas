...
proc summary data=bigjoin(where=(specchar = '*')) nway missing;
 class x y;
 var _number;
 output out=_prods(where=(_freq_ = 2)) sum=;
run;

proc sql;
 create table _prods_num as
 	select a.x, a.y, b._number from _prods as a
	inner join bigjoin as b
...
data part2;
 ...
 if first.x then value1 = _number;
 else prod = _number * value1;
run;
